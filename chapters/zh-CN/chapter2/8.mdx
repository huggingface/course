	...
	choices={[
		{
			...
			explain: "错误。你可能把AutoModel与我们的<a href='https://huggingface.co/autonlp'>AutoNLP</a> 产品相混淆了？"
		},
		{
		...
		},
		{
			...
			explain: "不正确; 虽然有些Checkpoint(检查点)和模型能够处理多种语言，但是没有内置的工具可以根据语言自动选择Checkpoint(检查点)。你应该前往 <a href='https://huggingface.co/models'>Model Hub</a> 寻找完成所需任务的最佳Checkpoint(检查点)！"
		} 
	]}
/>
...
<Question
	choices={[
		{
			text: "根据你的数据自动进行训练的模型",
			explain: "错误。你可能把TFAutoModel与我们的<a href='https://huggingface.co/autonlp'>AutoNLP</a> 产品相混淆了？"
		},
		{
			text: "一个根据Checkpoint(检查点)返回模型体系结构的对象",
