...
# Train in mixed-precision float16 [[Train in mixed-precision float16]]
# Comment this line out if you're using a GPU that will not benefit from this [[Comment this line out if you're using a GPU that will not benefit from this]]
tf.keras.mixed_precision.set_global_policy("mixed_float16")

# The number of training steps is the number of samples in the dataset, divided by the batch size then multiplied [[The number of training steps is the number of samples in the dataset, divided by the batch size then multiplied]]
# by the total number of epochs. Note that the tf_train_dataset here is a batched tf.data.Dataset, [[by the total number of epochs. Note that the tf_train_dataset here is a batched tf.data.Dataset,]]
# not the original Hugging Face Dataset, so its len() is already num_samples // batch_size. [[not the original Hugging Face Dataset, so its len() is already num_samples // batch_size.]]
num_epochs = 3
