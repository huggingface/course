...
Transformer架构最初是为翻译而设计的。在训练期间，编码器接收特定语言的输入（句子），而解码器接收相同的句子，但是目标语言已经被转换为所需的语言。在编码器中，注意力层可以使用一个句子中的所有单词（正如我们刚才看到的，给定单词的翻译可以取决于它在句子中的其他单词）。然而，解码器是按顺序工作的，并且只能注意它已经翻译过的句子中的单词。例如，当我们预测了翻译目标的前三个单词时，我们将它们提供给解码器，然后解码器使用编码器的所有输入来尝试预测第四个单词。
...
也可以在编码器/解码器中使用*注意力遮罩层*，以防止模型注意某些特殊单词。例如，用于在批处理句子时，使所有输入具有相同长度的特殊填充词。
