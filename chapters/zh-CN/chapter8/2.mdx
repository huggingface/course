...
- 如果最后一行没有包含足够的信息, 请按照你的方式进行回溯, 看看你是否可以确定源代码中发生错误的位置。
...
尽管 `pipeline` 对于大多数需要快速生成预测的应用程序来说非常有用, 有时你需要访问模型的 logits (例如, 如果你有一些想要应用的自定义后处理)。为了看看在这种情况下会出现什么问题, 让我们首先从 `pipeline` 中获取模型和标记器:
...
噢, 看起来我们的代码中有一个错误!但我们不怕一点调试。你可以在笔记本中使用 Python 调试器:
