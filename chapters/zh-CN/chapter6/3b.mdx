...
我们现在将深入研究 **question-answering** 管道，看看如何利用偏移量从上下文中获取手头问题的答案，有点像我们在上一节中对分组实体所做的。然后我们将看到我们如何处理最终被截断的非常长的上下文。如果你对问答任务不感兴趣，可以跳过此部分。
...
✏️ **试试看!** 使用你之前计算的最佳分数来显示五个最可能的答案。要检查你的结果，请返回到第一个管道并在调用它时传入。
...
现在让我们回到我们的长期背景。默认情况下 **question-answering** 管道使用的最大长度为 384，正如我们之前提到的，步长为 128，这对应于模型微调的方式（你可以通过传递 **max_seq_len** 和 **stride** 调用管道时的参数）。因此，我们将在标记化时使用这些参数。我们还将添加填充（具有相同长度的样本，因此我们可以构建张量）以及请求偏移量：
...
✏️ **试试看!** 使用你之前计算的最佳分数来显示五个最可能的答案（对于整个上下文，而不是每个块）。要检查你的结果，请返回到第一个管道并在调用它时传入。

</Tip>

我们对分词器功能的深入研究到此结束。我们将在下一章再次将所有这些付诸实践，届时我们将向你展示如何在一系列常见的 NLP 任务上微调模型。
