...
例如，正如我们在 [上一节](/course/chapter9/2) 中看到的，你可以传入一个 `Textbox(lines=7, label="Prompt")` 组件来创建一个包含 7 行和一个标签的文本框，而不是将 `"textbox"` 传递给 `inputs` 参数。
...
像往常一样，我们将使用 🤗 Transformers 中的 `pipeline()` 函数加载我们的语音识别模型。如果你需要快速复习，你可以返回 [第 1 章中的那个部分](/course/chapter1/3)。 接下来，我们将实现一个 `transcribe_audio()` 函数来处理音频并返回转录。 最后，我们将把这个函数包装在一个 `Interface` 中，其中 `Audio` 组件用于输入，只有文本用于输出。 总而言之，此应用程序的代码如下：
