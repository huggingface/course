...
<Question
	choices={[
        {
			text: "为你的机器学习模型创建一个演示",
			explain: "通过几行 Python 代码，你可以使用我们的预构建组件库为 ML 模型生成一个演示。",
			correct: true
		},
		{
			text: "与他人分享你的机器学习模型",
		...
		{
			text: "调试模型",
			explain: "Grado演示的一个优点是能够用真实数据测试模型，你可以实时更改并观察模型的预测变化，从而帮助你调试模型。",
			correct: true
		},
		{
			text: "训练你的模型",
...
<Question
	choices={[
        ...
        {
			text: "不正确",
			explain: "Grdio 是不依赖模型的，这意味着你可以为任何类型的机器学习模型创建演示。",
			correct: true
        }
	]}
/>
	...
	choices={[
        {
			...
			explain: "对于你最喜欢的 IDE，Grdio 工作得非常好。",
            correct: true
        },
        {
        ...
        {
			text: "Jupyter笔记本",
			explain: "很好的选择 —— 你可以在你的Jupyter notebook创建并启动一个演示。",
			correct: true
        }
	]}
/>
...
<Question
	choices={[
        ...
        },
        {
		 ...
		 {
			text: "创建多步演示; 这意味着允许你重用一个组件的输出作为下一个组件的输入",
