...
我们鼓励所有训练模型的用户通过与社区共享来做出贡献——共享模型，即使是在非常特定的数据集上进行训练，也将帮助他人，节省他们的时间和计算资源，并提供对有用的训练工件的访问。反过来，你可以从其他人所做的工作中受益！
...
创建存储库后，你可以通过 git 和 git-lfs 将文件上传到其中。我们将在以下部分引导你创建模型存储库并将文件上传到它们
...
在继续之前，你需要生成一个身份验证令牌，以便 **huggingface_hub** API 知道你是谁以及你对哪些名称空间具有写入权限。确保你在一个环境中 **transformers** 已安装（见[Setup](/course/chapter0)）。如果你在笔记本中，可以使用以下功能登录：
...
在这两种情况下，系统都会提示你输入用户名和密码，这与你用于登录 Hub 的用户名和密码相同。如果你还没有 Hub 配置文件，则应该创建一个[here](https://huggingface.co/join)。

好的！你现在已将身份验证令牌存储在缓存文件夹中。让我们创建一些存储库！
...
你声明 **trainer.train()** 的时候， 这 **Trainer** 然后每次将你的模型保存到你的命名空间中的存储库中时（这里是每个时代），它将上传到集线器。该存储库将命名为你选择的输出目录（此处 **bert-finetuned-mrpc** ) 但你可以选择不同的名称 **hub_model_id = a_different_name** 。

要将你的模型上传到你所属的组织，只需将其传递给 **hub_model_id = my_organization/my_repo_name** 。
...
如果你使用Keras来训练你的模型，则将其上传到Hub的最简单方法是在调用 **model.fit()**  时传递**PushToHubCallback**：
...
然后，你应该在对**model.fit()**的调用中添加**callbacks=[callback]**。然后，每次将模型保存在命名空间的存储库中（此处为每个 epoch）时，回调都会将模型上传到 Hub。该存储库的名称将类似于你选择的输出目录（此处为**bert-finetuned-mrpc**），但你可以选择另一个名称，名称为**hub_model_id = a_different_name**。
...
你可以自由地用这些做任何你想做的事情——向标记器添加标记，训练模型，微调它。一旦你对生成的模型、权重和标记器感到满意，你就可以利用 **push_to_hub()** 方法直接在 **model** 中：

```py
model.push_to_hub("dummy-model")
```

这将创建新的存储库 **dummy-model** 在你的个人资料中，并用你的模型文件填充它。
...
如果你属于一个组织，只需指定 **organization** 上传到该组织的命名空间的参数：

```py
tokenizer.push_to_hub("dummy-model", organization="huggingface")
```

如果你希望使用特定的 Hugging Face 令牌，你可以自由地将其指定给 **push_to_hub()** 方法也是：

```py
tokenizer.push_to_hub("dummy-model", organization="huggingface", use_auth_token="<TOKEN>")
```

现在前往模型中心找到你新上传的模型：*https://huggingface.co/user-or-organization/dummy-model*。

单击“文件和版本”选项卡，你应该会在以下屏幕截图中看到可见的文件：
...
✏️ **试试看**！获取与检查点关联的模型和标记器，并使用该方法将它们上传到你的命名空间中的存储库。在删除之前，请仔细检查该存储库是否正确显示在你的页面上。

</Tip>

如你所见， **push_to_hub()** 方法接受多个参数，从而可以上传到特定的存储库或组织命名空间，或使用不同的 API 令牌。我们建议你查看直接在[🤗 Transformers documentation](https://huggingface.co/transformers/model_sharing.html)了解什么是可能的

这 **push_to_hub()** 方法由[huggingface_hub](https://github.com/huggingface/huggingface_hub)Python 包，为 Hugging Face Hub 提供直接 API。它集成在 🤗 Transformers 和其他几个机器学习库中，例如[allenlp](https://github.com/allenai/allennlp).虽然我们在本章中专注于 🤗 Transformers 集成，但将其集成到你自己的代码或库中很简单。
...
类似于使用 **push_to_hub** API，这将要求你将 API 令牌保存在缓存中。为此，你需要使用 **login** 来自 CLI 的命令，如上一节所述（同样，确保在这些命令前面加上 **!** 字符（如果在 Google Colab 中运行）：
...
这将创建存储库 **dummy-model** 在你的命名空间中。如果愿意，你可以使用 **organization** 争论：
...
这将创建 **dummy-model** 存储库中的 **huggingface** 命名空间，假设你属于该组织。
其他可能有用的参数是：

- private 以指定存储库是否应对其他人可见。
- token 如果你想用给定的令牌覆盖存储在缓存中的令牌。
...
Web 界面提供了直接在 Hub 中管理存储库的工具。使用该界面，你可以轻松创建存储库、添加文件（甚至是大文件！）、探索模型、可视化差异等等。
...
首先，指定存储库的所有者：这可以是你或你所属的任何组织。如果你选择一个组织，该模型将出现在该组织的页面上，并且该组织的每个成员都可以为存储库做出贡献。

接下来，输入你的模型名称。这也将是存储库的名称。最后，你可以指定你的模型是公开的还是私有的。私人模特要求你拥有付费 Hugging Face 帐户，并允许你将模特隐藏在公众视野之外。
...
这是你的模型将被托管的地方。要开始填充它，你可以直接从 Web 界面添加 README 文件。
...
README 文件在 Markdown 中 - 随意使用它！本章的第三部分致力于构建模型卡。这些对于为你的模型带来价值至关重要，因为它们是你告诉其他人它可以做什么的地方。

如果你查看“文件和版本”选项卡，你会发现那里还没有很多文件——只有自述文件你刚刚创建和.git 属性跟踪大文件的文件。
...
使用 **upload_file** 不需要在你的系统上安装 git 和 git-lfs。它使用 HTTP POST 请求将文件直接推送到 🤗 Hub。这种方法的一个限制是它不能处理大于 5GB 的文件。
如果你的文件大于 5GB，请按照下面详述的另外两种方法进行操作。API 可以按如下方式使用：
...
另外！我们建议你查看 **Repository** 可用文件[here](https://github.com/huggingface/huggingface_hub/tree/main/src/huggingface_hub#advanced-programmatic-repository-management)有关所有可用方法的概述。
...
如果你刚刚使用 Hugging Face Hub 创建了你的存储库 **create_repo** 方法，这个文件夹应该只包含一个隐藏的 **.gitattributes** 文件。如果你按照上一节中的说明使用 Web 界面创建存储库，则该文件夹应包含一个自述文件文件旁边的隐藏 **.gitattributes** 文件，如图所示。
...
如果你查看文件大小（例如， **ls -lh** )，你应该会看到模型状态 dict 文件 (pytorch_model.bin) 是唯一的异常值，超过 400 MB。

{/if}

<Tip>
✏️ 从 web 界面创建存储库时，*.gitattributes* 文件会自动设置为将具有某些扩展名的文件，例如 *.bin* 和 *.h5* 视为大文件，git-lfs 会对其进行跟踪你无需进行必要的设置。
</Tip> 
...
推送可能需要一些时间，具体取决于你的互联网连接速度和文件大小：
