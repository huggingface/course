...
1. Non è stato detto al `Trainer` di valutare il modello durante l'addestramento, settando `eval_strategy` o al valore `"steps"` (valuta il modello ogni `eval_steps`) oppure al valore `"epoch"` (valuta il modello alla fine di ogni epoca).
...
Da notare che bisogna creare un nuovo oggetto `TrainingArguments` con il valore di `eval_strategy` pari a `"epoch"` e un nuovo modello — altrimenti si continuerebbe l'addestramento del modello già addestrato. Per lanciare una nuova esecuzione dell'addestramento si usa:
