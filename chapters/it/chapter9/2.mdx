...
Iniziamo con un semplice esempio di "Hello World" per prendere familiarità con la sintassi di Gradio:
...
Si potrebbe fare lo stesso per la casella di testo in output, ma per ora ci limiteremo a questo.
...
Costruiamo ora una semplice interfaccia che consenta di dimostrare un modello di **generazione del testo**  come GPT-2.
...
Per prima cosa, definiamo una funzione di predizione che riceve una richiesta di testo e restituisce il testo completato:
...
Questa funzione completa le richieste fornite dall'utente e puoi eseguirla con qualche tuo input per vedere come funziona. Ecco un esempio (si potrebbe ottenere un risultato diverso):
