...
모델이 준비되면, 지금까지 구성한 모든 객체들(`model`, `training_args`, 훈련 및 검증 데이터셋, `data_collator`, `processing_class`)를 전달하여 `Trainer`를 정의할 수 있습니다. `processing_class` 매개변수는 비교적 최근에 추가된 기능으로, `Trainer`에게 어떤 토크나이저를 사용해 데이터를 처리할지 알려줍니다:
...
각 에폭이 끝날 때마다 메트릭을 보고하기 위해 이 `compute_metrics()` 함수가 실제로 어떻게 사용되는지 보려면, 다음과 같이 새 `Trainer`를 정의하면 됩니다:
