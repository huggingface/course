...
In [Chapter 1](/course/chapter1), we used Transformer models for different tasks using the high-level `pipeline` API. Although this API is powerful and convenient, it's important to understand how it works under the hood so we have the flexibility to solve other problems.
