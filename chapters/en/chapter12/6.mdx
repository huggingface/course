...
In this exercise, you'll fine-tune a model with GRPO using Unsloth. 

Unsloth is a library that accelerates LLM fine-tuning, making it possible to train models faster and with less computational resources. Unsloth is plugs into TRL, so we'll build on what we learned in the previous sections.

We'll use Unsloth to implement GRPO (Group Relative Policy Optimization) to improve a model's reasoning capabilities. We covered GRPO in [Chapter 3](/en/chapter3/3).

<Tip>
This exercise can be run on a free Google Colab T4 GPU. For the best experience, follow along with the notebook linked above.
</Tip>
...
Unsloth provides a special patch for reinforcement learning algorithms like GRPO. We need to apply this patch before using any RL functionality:
...
This code loads the model in 4-bit quantization to save memory and applies LoRA (Low-Rank Adaptation) for efficient fine-tuning. The `target_modules` parameter specifies which layers to fine-tune, and `use_gradient_checkpointing` enables training with longer contexts.
...
First, we will define the format we want the model to follow:
...
As we discussed in [Chapter 3](/en/chapter3/3), GRPO requires reward functions to guide the model's learning. In this exercise, we'll define several reward functions that encourage different aspects of good reasoning:
