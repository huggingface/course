...
##Understanding pip tool[download-external-libraries]
If you are new to Python, `pip` is the tool that Python uses to install external libraries.
Libraries like `transformers` or `datasets` are not included with Python by default,
so we use `pip` to download and manage them.
For example:
```bash
pip install transformers
```
This command downloads the library and makes it available for use in your Python programs.
