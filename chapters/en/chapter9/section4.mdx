...
We'll cover both of those approach shortly. But before you share your demo, you may want to polish it up... 

### Polishing your `gradio` demo:

<img src="/course/static/chapter9/gradio-demo-overview.png" alt="Overview of a gradio interface" width="100%">

  - How can we add more to our demos? The `Interface` class supports some optional parameters:
...
Using (some of) these options, we end up with a more complete interface like the one below:
...
We can load the labels from [class_names.txt](https://huggingface.co/spaces/dawood/Sketch-Recognition/blob/main/class_names.txt) and load the pre-trained pytorch model from [pytorch_model.bin](https://huggingface.co/spaces/dawood/Sketch-Recognition/blob/main/pytorch_model.bin). The rest of the code to load the model and create the Interface is shown below:
