...
<img src="/course/static/chapter9/gradio-demo-overview-dark.png" alt="Overview of a gradio interface" width="100%">

  - As you can see in the image above, we can add additional content to our demos. The `Interface` class supports some optional parameters:
...
Using the options above (besides `live`), we end up with a more complete interface. Try the interface below:
...
Hugging Face Spaces provides the infrastructure to permanently host your Gradio model on the internet, **for free**! Spaces allows you to create and push to a repo, where your gradio 
interface code will exist in an `app.py` file. [Read a tutorial](https://huggingface.co/blog/gradio-spaces) to get started, or watch an example video below.

<Youtube id="LS9Y2wDVI0k" />

## ✏️ Let's apply it!

Using what we just learned in the sectios so far, let's create the sketch recognition demo we saw in [section one of this chapter](/course/chapter9/section1). Let's add some customization to our interface and set `share=True` to create a public link we can pass around.
...
a prediction every time someone draws on the sketchpad (no submit button!). Furthermore, we also
set the `share=True` argument in the `launch()` method. This will create a public link that you can 
...
Next up, in section 5, we cover other ways `gradio` can be used with Hugging Face!
