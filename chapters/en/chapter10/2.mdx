...
* **Audio clasification:** Classify audio clips into different categories, e.g. "dog" or "cat" for a clip of a dog barking or a cat meowing.
* **Automatic speech recognition:** Transcribe audio clips into text, e.g. "How are you doing today?" for a clip of someone speaking.
* **Speaker diarization:** Identify which speaker is speaking at any given time in an audio clip, e.g. "Alice" or "Bob" for a clip of two people talking.
* **Automative speaker verification:** Verify whether the speakers of a pair of utterances are the same.
...
For this example, we'll use [MINDS-14 dataset](https://huggingface.co/datasets/PolyAI/minds14), which contains recordings of people asking questions to an e-banking system in several languages and dialects. The dataset is small and easy to use, so it's a great place to start. Let's begin by downloading the Australian subset of recordings from the Hub by using the `load_dataset()` function from 🤗 Datasets:
...
The dataset contains 654 audio files, each of which is accompanied with a transcription, an English translation, and a label indicating the _intent_ behind the person's query. The `audio` column contains the raw audio data, so let's take a closer look at one of the examples:
...
>>> Add your turn for a language of choice?
