...
Similar to tokenization in NLP, audio and speech Transformers require the input to be encoded in a format that the model can understand. For models like Wa2Vec2 and HuBERT, the input is simply the raw waveform, but for others like Whisper and MCTC, the input is a spectrogram. In 🤗 Transformers, the conversion from audio to the input format is handled by the _feature extractor_ of the model. Similar to tokenizers in NLP, 🤗 Transformers provides a convenient `AutoFeatureExtractor` class that can automatically select the correct feature extractor for a given model. To see how we can process our audio files, let's begin by instantiating the feature extractor for DistilHuBERT:
...
and then cast the dataset using the `Audio` feature from 🤗 Datasets:
...
One last thing to take care of: length of songs.
