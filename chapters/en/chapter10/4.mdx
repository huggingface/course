...
A summary of datasets on the HF Hub:
...
raw_dataset["train"] = load_dataset("librispeech_asr", "clean", split="train.100")
raw_dataset["validation"] = load_dataset("librispeech_asr", "clean", split="validation")
raw_dataset["test"] = load_dataset("librispeech_asr", "clean", split="test")
```
...
Slap bang in the middle of the dataset card we have exactly what we're looking for: the dataset viewer! The dataset viewer shows us the first 100 samples of any dataset. What's more, it's loaded up the audio samples for us to listen to in real-time! If we hit the play button on the first sample, we can listen to the audio and see the corresponding text. Have a scroll through the samples for the train and test sets to get a better feel for the audio data that we're dealing with.
...
We're already pretty familiar with the tokenizer: it converts the text data into a set of token IDs that can be interpreted by the model.

But the feature extractor is new! Simply put, the feature extractor prepares our audio data for the model.
...
So, what does the feature extractor do exactly?

Well, the audio data is in the right format, but we've imposed no restrictions on the values it can take.

For our model to work optimally, we want to keep all the inputs within the same dynamic range.

This is going to make sure we get a similar range of activations and gradients for our samples, helping with model stability and convergence.

To do this, we _normalise_ our audio data, by rescaling each sample to zero mean and unit variance, a process called _feature scaling_.

It's exactly this feature normalisation that our feature extractor performs!
...
sample = raw_dataset["train"][0]["audio"]
...
```
We can see that the mean is close to zero already, but the standard deviation is a long way off! This would cause our model problems, as the dynamic range of the audio data would be very small and hard to make out. Let's apply the feature extractor and see what the outputs look like:
