...
GRPO uses preference data directly like DPO, but also compares groups of similar samples to improve stability and sample efficiency.

GRPO does not explicitly use a reward model, but a reward model can be used to give a score to the group of responses.

In fact, GRPO can incorporate reward signals from any function or model. For example, we could use a length function to reward shorter responses or a factual correctness function to reward responses that are more factually correct.
