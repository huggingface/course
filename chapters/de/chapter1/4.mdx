...
Hier ist ein Projekt zu sehen, bei dem ein Team gezielt versucht, die Umweltauswirkungen des Pretrainings (sehr großer) Modelle zu reduzieren. Wenn man die vielen Versuche berücksichtigt, die dazu nötig sind, die besten Hyperparameter zu finden, wären die zu bemessenden ökologischen Konsequnezen noch größer.
...
* Das vortrainierte Modell wurde bereits auf einem Datensatz trainiert, der einige Ähnlichkeiten mit dem Datensatz, der für das Feintuning verwendet wird, aufweist. Beim Feintuning kann also von dem Wissen profitiert werden, das das ursprüngliche Modell während des Pretrainings erlangt hat (bei NLP-Problemstellungen verfügt das vortrainierte Modell zum Beispiel über eine Art statistisches Verständnis der Sprache, die du für deine Aufgabe verwendest).
...
* **Encoder (links)**: Der Encoder, auch Kodierer genannt, empfängt einen Input und erstellt eine Repräsentation des Inputs (seiner Features, im Deutschen auch als Merkmale bezeichnet). Das bedeutet, dass das Modell darauf optimiert ist, ein Verständnis(#M acquire understanding from#) vom Input zu erlangen.
* **Decoder (rechts)**: Der Decoder, auch bekannt als Dekodierer, verwendet die Repräsentation des Encoders (Features) zusammen mit anderen Inputs, um eine Zielsequenz zu generieren. Das bedeutet, dass das Modell darauf optimiert ist, Outputs(#M einen Output#) zu generieren.
...
* **Encoder-Decoder-Modelle** bzw. **Sequence-to-Sequence-Modelle**: Gut für generative Aufgaben, die einen Input erfordern, wie z. B. Übersetzungen oder Zusammenfassungen.
...
Um das Training zu beschleunigen (insofern das Modell Zugang zu den Zielsätzen hat), wird der Decoder mit dem gesamten (vorherzusagenden) Zielsatz gefüttert, aber er darf keine nachfolgenden Wörter verwenden (wenn er Zugang zum Wort an Position 2 hätte, während er versucht, das Wort an Position 2 vorherzusagen, wäre die Aufgabe nicht sonderlich schwer!). Wenn er zum Beispiel versucht, das vierte Wort vorherzusagen, hat der Attention Layer nur Zugang zu den Wörtern an den Positionen 1 bis 3.
...
Beachte, dass der erste Attention Layer in einem Decoder-Block seine Aufmerksamkeit auf alle (vorangegangenen/vorherigen/vergangen #M#) Inputs, die der Decoder erhält, richtet, während der zweite Attention Layer den Output des Encoders verwendet. Er kann also auf den gesamten Input-Satz zugreifen, um das aktuelle Wort bestmöglich vorherzusagen. Das ist sehr nützlich, da es in verschiedenen Sprachen unterschiedliche grammatikalische Regeln geben kann, die die Wörter in eine andere Reihenfolge bringen(#M wodurch die Wörter in einer anderen Reihenfolge aneinandergereiht werden; put words in different orders#), oder ebenso kann ein später im Satz enthaltener Kontext hilfreich sein, um die beste Übersetzung eines bestimmten Wortes zu bestimmen.
