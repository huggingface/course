...
其中最大的問題是，為了能夠在大量資料上進行預訓練，研究人員通常會抓取所有能找到的內容，既包含網路上最好的內容，也包含最糟糕的內容。
...
即使 BERT 是少數不是透過抓取整個網路資料建構的 Transformer 模型之一，而是使用看似中性的資料（它是在英文維基百科和 BookCorpus 資料集上訓練的），這種情況仍然會發生。