...
1. Nu am precizat că `Trainer` trebuie să evalueze în timpul antrenamentului prin setarea `eval_strategy` la `„steps”` (evaluare la fiecare `eval_steps`) sau `„epoch”` (evaluare la sfârșitul fiecărei epoch).
...
Rețineți că creăm un nou `TrainingArguments` cu `eval_strategy` setat la `„epoch”` și un nou model - în caz contrar, am continua doar instruirea modelului pe care l-am instruit deja. Pentru a lansa o nouă rulare de formare, executăm:
