...
この章で学んだことを一緒に試してみましょう！

### 1. Pythonのトレースバックはどのような順番で読むべきですか？

<Question
	choices={[
		{
			text: "上から下へ",
			explain: "もう一回！他のほとんどのプログラミング言語では、例外は先頭に表示されますが、Pythonはこの点で特別です。"
		},
		{
			text: "下から上へ",
			explain: "正解です! Pythonのトレースバックで例外が一番下に表示されることの利点は、ターミナルで作業しているときに、この行が最後に表示されたときにデバッグしやすくなることです。",
			correct: true
		}
	]}
/>
