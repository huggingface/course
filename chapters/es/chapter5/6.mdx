...
Como mencionamos con anterioridad, queremos representar cada entrada en el corpus de issues de GitHub como un vector individual, así que necesitamos agrupar o promediar nuestros _embeddings_ de tokes de alguna manera. Un abordje popilar es ejecutar *CLS pooling* en los outputs de nuestro modelo, donde simplemente vamos a recolectar el último estado oculto para el token especial `[CLS]`. La siguiente función nos ayudará con esto:
...
Ahora crearemos una función que va a tokenizar una lista de documentos, ubicar los rensores en la GPU, alimentarlas al modelo y aplicar CLS pooling a los outputs:
...
Los _embeddings_ se han convertido en arrays de NumPy, esto es porque 🤗 Datasets los necesita en este formato cuando queremos indexzzarlos con FAISS, que es lo que haremos a continuación.
...
Tal como en los documentos, ahora tenemos un vector de 768 dimensiones que represent ala pregunta, que podemos comparar con el corpus entero para encontrar los _embeddings_ más parecidos:
