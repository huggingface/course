...
Asumiendo que los eventos "La respuesta comienxa en `start_index`" y "La respuesta termina en `end_index`" son independientes, la probabilidad que la respuesta inicio en `start_index` y termine en `end_index` es:
...
Luego enmascararemos los valores donde `start_index > end_index` reemplazandolos como 0 (las otras probabilidades son todos números positivos). La función `torch.triu()` retorna la parte triangular superior de el tensor 2D pasado como argumento, por lo que hará el enmascaramiento por nosotros. 
...
Luego enmascararemos los valores donde `start_index > end_index` reemplazandolos como 0 (las otras probabilidades son todos números positivos). La función `np.triu()` retorna la parte triangular superior de el tensor 2D pasado como argumento, por lo que hará el enmascaramiento por nosotros. 
...
No estamos listos aún, pero al menos ya tenemos el puntaje correcto para la respuesta (puedes chequear esto comparandolo con el primer resultado en la sección previa):
...
✏️ **Inténtalo!** Calcula los índice de los índice de inicio y términi para las cinco respuestas más probables.
...
Como podemos ver, la oración ha sido dividida en trozos de tal manera que cada entrada en `inputs["input_ids"] tiene a lo más 6 tokens (tendríamos que agregar padding en el último trozo para tener el mismo largo que los otros) y hay traslape (overlap) de 2 tokens entre cada uno de los trozos. 
...
Nuestro contexto largo fue dividido en dos, lo que significa que después de pasar por nuestro modelo, tendremos 2 sets logits de inicio y término:
...
Al igual que antes, primero enmascaramos los tokens que no son parte del contexto antes de aplicar softmax. También enmascaramos todos los tokens de padding (de acuerdo a la máscara de atención (attention masks)):
...
Si ignoramos el primer resultado, obtenemos el mismo resultado que nuestro pipeline para el contexto largo -- yay!

<Tip>

✏️ **Inténtalo!** Usa los mejores puntajes que calculaste antes para mostrar las 5 respuestas más probables. Para revisar tus resultados, regresa al primer pipeline y agrega `top_k=5`Use al llamarlo.

</Tip>

Esto concluye nuestra profundización en las capacidades de los tokenizadores. Pondremos todo esto en práctica de nuevo en el siguiente capítulo, cuando te mostremos cómo hacer fine-tuning a un modelo en una variedad de tareas comunes de NLP.
