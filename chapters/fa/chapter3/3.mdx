...
با داشتن مدل، ما می‌توانیم `Trainer`ای را با ارسال همه اشیائی که تا کنون ساخته شده‌اند - `training_args`،  `model`، دیتاسِت‌های <span dir="ltr">`training` و `validation`</span>، `data_collator` و `tokenizer` به آن تعریف کنیم:
