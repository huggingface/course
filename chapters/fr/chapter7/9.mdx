	...
	choices={[
		{
			...
			explain: "Correct ! Nous pouvons alors étiqueter chaque mot comme étant un nom, un verbe, etc.",
			correct: true
		},
		{
			text: "Trouvez si une phrase est grammaticalement correcte ou non.",
			explain: "Non, c'est un problème de classification des séquences."
		},
		{
			text: "Trouvez les personnes mentionnées dans une phrase.",
			explain: "Correct ! Nous pouvons étiqueter chaque mot comme personne ou non personne.",
            correct: true
		},
        {
			text: "Trouvez le groupe de mots dans une phrase qui répond à une question.",
			explain: "Non, ce serait un problème de réponse aux questions."
		}
	]}
/>
	...
	choices={[
		{
			...
			explain: "Les textes sont en effet donnés comme des mots séparés, mais nous devons encore appliquer le modèle de tokenisation des sous-mots."
		},
		{
			text: "Les textes sont donnés sous forme de mots, il suffit donc d'appliquer la tokénisation en sous-mots.",
			explain: "C'est exact ! C'est différent du prétraitement habituel, où nous devons appliquer le pipeline complet de tokénisation. Pouvez-vous penser à une autre différence ?",
			correct: true
		},
		{
	...
	choices={[
		{
			...
			explain: "Nous étiquetons ces <code>-100</code> ils sont donc ignorés dans la perte."
		},
		{
		...
		},
		{
			...
			explain: "C'est incorrect. Nous avons besoin d'autant d'étiquettes que de <i>tokens</i>, sinon nos modèles se tromperont."
		}
	]}
/>
...
<Question
	choices={[
		{
			text: "C'est lorsque nous exécutons un modèle sur un jeu de données et que nous obtenons les prédictions pour chaque échantillon de cet ensemble de données.",
			explain: "Non, c'est juste une déduction courante."
		},
		{
			text: "C'est quand on entraîne un modèle sur un jeu de données.",
			explain: "Non, il s'agit d'entraîner un modèle. Il n'y a pas d'adaptation ici."
		},
		{
			text: "C'est lorsque nous affinons un modèle pré-entraîné sur un nouvel ensemble de données, et qu'il donne des prédictions qui sont plus adaptées à ce jeu de données.",
			explain: "Correct ! Le modèle a adapté ses connaissances au nouvel jeu de données.",
            correct: true
		},
        {
...
<Question
	choices={[
		...
		{
			text: "Certains des <i>tokens</i> de la phrase d'entrée sont masqués aléatoirement et les étiquettes sont les <i>tokens</i> d'entrée originaux, décalés vers la gauche.",
		...
		},
		{
			...
			explain: "Il s'agit d'un problème de classification de séquences avec une certaine augmentation des données, et non d'une modélisation du langage masqué."
		},
        {
	...
	choices={[
		{
			...
			explain: "Oui, c'est un problème de résumé. Essayez une autre réponse !",
            correct: true
		},
		{
			text: "Répondre à des questions sur un document",
			explain: "Cela peut être formulé comme un problème de séquence à séquence. Ce n'est pas la seule bonne réponse, cependant.",
            correct: true
		},
		{
...
<Question
	choices={[
		...
		},
		{
			...
			explain: "C'est exact. Les prédictions des modèles de séquence à séquence sont souvent exécutées en utilisant la méthode <code>generate()</code>.",
			correct: true
		},
		{
			text: "Parce que les cibles sont des textes dans des problèmes de séquence à séquence",
			explain: "Le <code>Trainer</code> ne se soucie pas vraiment de cela puisqu'ils ont été prétraités auparavant."
		},
        {
...
<Question
	choices={[
		{
			text: "Parce que les <i>tranformers</i> sont entraînés avec un apprentissage non supervisé.",
			explain: "Pas tout à fait. Même l'apprentissage non supervisé a besoin d'une fonction de perte !"
		},
		{
		...
		},
        {
			...
			explain: "Non, la fonction de perte est toujours fixée une fois que vous exécutez `model.compile()`, et ne peut pas être modifiée dans `model.fit()`."
		}
	]}
/>
...
<Question
	choices={[
		...
		},
		{
			...
			explain: "C'est exact, la modélisation du langage est un problème autosupervisé.",
			correct: true
		},
		{
	...
	choices={[
		{
			...
			explain: "C'est exact, mais est-ce vraiment un défi majeur ?"
		},
		{
