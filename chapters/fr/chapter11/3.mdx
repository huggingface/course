<CourseFloatingBanner chapter={11}
  ...
  notebooks={[
    ...
    {label: "Français", value: "https://studiolab.sagemaker.aws/import/github/huggingface/notebooks/blob/master/course/fr/chapter11/section3.ipynb"},
]} />
...
- Vous avez besoin de performances supplémentaires par rapport à ce que les instructions peuvent réaliser.<br>
- Vus avez un cas d'utilisation spécifique pour lequel le coût d'utilisation d'un grand modèle polyvalent l'emporte sur le coût du *finetuning* d'un modèle plus petit<br>
...
<Tip>
Avant de commencer à utiliser le SFT, évaluez si votre cas d'utilisation nécessite :<br>
- un formatage précis des résultats<br>
- des connaissances spécifiques à un domaine<br>
- des schémas de réponse cohérents<br>
...
- Commencer avec 1-3 époques<br>
- Utiliser des batchs de plus petite taille dans un premier temps<br>
- Surveiller de près les métriques de validation<br>
- Ajuster le taux d'apprentissage si l'entraînement est instable<br>
</Tip>
...
<Tip warning={true}>
Soyez attentif à ces signes d'alerte pendant l'entraînement :<br>
1. La perte de validation augmente alors que la perte d'entraînement diminue (surentraînement)<br>
2. Pas d'amélioration significative des valeurs de la perte (sousentraînement)<br>
3. Valeurs de perte extrêmement faibles (mémorisation potentielle)<br>
	...
	choices={[
		{
			...
			explain: "C'est exact ! Ces paramètres déterminent la durée de l'entraînement du modèle, soit par le nombre d'époques, soit par le nombre total d'étapes.",
			correct: true
		},
		{
			text: "batch_size et learning_rate",
			explain: "Si ces éléments ont une incidence sur l'entraînement, ils n'en contrôlent pas directement la durée."
		},
		{
			text: "gradient_checkpointing et warmup_ratio",
			explain: "Ces paramètres ont une incidence sur l'efficacité et la stabilité de l'entraînement, et non sur sa durée."
		}
	]}
/>
...
<Question
    choices={[
        {
            text: "La perte de validation augmente tandis que la perte d'entraînement continue à diminuer",
            explain: "C'est exact ! Cette divergence entre les pertes d'entraînement et de validation est un signe classique de surentraînement.",
            correct: true
        },
        {
            text: "Les pertes liées à l'entraînement et à la validation diminuent régulièrement",
            explain: "Ce schéma indique en fait un entraînement sain."
        },
        {
            text: "La perte d'entraînement reste constante alors que la perte de validation diminue",
            explain: "Il s'agit d'un schéma inhabituel qui n'indique pas un surentraînement."
        }
    ]}
/>
...
<Question
    choices={[
        {
            text: "Pour augmenter la taille effective des batchs sans utiliser plus de mémoire",
            explain: "C'est exact ! Il accumule les gradients sur plusieurs passages avant de mettre à jour les poids.",
            correct: true
        },
        {
            text: "Pour enregistrer des checkpoints pendant l'entraînement",
            explain: "Ceci est géré par les paramètres save_steps et save_strategy."
        },
        {
            text: "Pour contrôler le programme du taux d'apprentissage",
            explain: "Le programme du taux d'apprentissage est contrôlée par learning_rate et warmup_ratio."
        }
    ]}
/>
...
<Question
    choices={[
        {
            text: "Des mesures quantitatives et des résultats qualitatifs",
            explain: "C'est exact ! Le contrôle des deux types de métriques permet de détecter tous les problèmes potentiels.",
            correct: true
        },
        {
            text: "Seulement la perte d'entraînement",
            explain: "La perte d'entraînement ne suffit pas à garantir un bon comportement du modèle."
        },
        {
            text: "Seulement la qualité des sorties du modèle",
            explain: "Bien qu'importante, l'évaluation qualitative seule passe à côté d'une dynamique d'entraînement importante."
        }
    ]}
/>
...
<Question
    choices={[
        {
            text: "Un faible écart entre les pertes d'entraînement et de validation",
            explain: "C'est exact ! Cela indique que le modèle apprend des schémas généralisables.",
            correct: true
        },
        {
            text: "La perte d'entraînement atteint zéro",
            explain: "Des valeurs de perte extrêmement faibles peuvent indiquer une mémorisation plutôt qu'un apprentissage."
        },
        {
            text: "Perte de validation inférieure à la perte d'entraînement",
            explain: "Cela serait inhabituel et pourrait indiquer des problèmes avec l'ensemble de validation."
        }
    ]}
/>
