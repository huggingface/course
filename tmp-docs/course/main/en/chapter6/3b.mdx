...
local: qa
sections:
- local: questionanswering
  title: 使用 `question-answering` 管道
- local: ''
  title: 使用模型进行问答
- local: ''
  title: 处理长上下文
...
<h2 id="questionanswering">使用 `question-answering` 管道</h2>
...
```python out
&amp;lcub;'answer': 'Jax, PyTorch and TensorFlow',
 'start': 78,
 'end': 105,
 'score': 0.97773}
```
...
for candidate, offset in zip(candidates, offsets):
    ...
    result = &amp;lcub;"answer": answer, "start": start_char, "end": end_char, "score": score}
    print(result)
```

```python out
&amp;lcub;'answer': '\n🤗 Transformers: State of the Art NLP', 'start': 0, 'end': 37, 'score': 0.33867}
&amp;lcub;'answer': 'Jax, PyTorch and TensorFlow', 'start': 1892, 'end': 1919, 'score': 0.97149}
```
