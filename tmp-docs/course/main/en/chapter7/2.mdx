...
<h3 id="conll2003">CoNLL-2003 数据集</h3>
...
<h2 id="trainer-api">使用 Trainer API 微调模型</h2>

使用 `Trainer` 的实际代码会和以前一样；唯一的变化是数据整理成时批处理的方式和度量计算函数。

{:else}

<h2 id="keras">使用 Keras 微调模型</h2>
...
```py
id2label = &amp;lcub;str(i): label for i, label in enumerate(label_names)}
label2id = &amp;lcub;v: k for k, v in id2label.items()}
```
...
```python out
&amp;lcub;'MISC': &amp;lcub;'precision': 1.0, 'recall': 0.5, 'f1': 0.67, 'number': 2},
 'ORG': &amp;lcub;'precision': 1.0, 'recall': 1.0, 'f1': 1.0, 'number': 1},
...
```python out
&amp;lcub;'LOC': &amp;lcub;'precision': 0.91, 'recall': 0.92, 'f1': 0.91, 'number': 1668},
 'MISC': &amp;lcub;'precision': 0.70, 'recall': 0.79, 'f1': 0.74, 'number': 702},
 'ORG': &amp;lcub;'precision': 0.85, 'recall': 0.90, 'f1': 0.88, 'number': 1661},
 'PER': &amp;lcub;'precision': 0.95, 'recall': 0.95, 'f1': 0.95, 'number': 1617},
...
for epoch in range(num_train_epochs):
    ...
    print(
        f"epoch &amp;lcub;epoch}:",
        &amp;lcub;
            key: results[f"overall_&amp;lcub;key}"]
    ...
    if accelerator.is_main_process:
        ...
        repo.push_to_hub(
            commit_message=f"Training in progress epoch &amp;lcub;epoch}", blocking=False
        )
```
...
```python out
[&amp;lcub;'entity_group': 'PER', 'score': 0.9988506, 'word': 'Sylvain', 'start': 11, 'end': 18},
 &amp;lcub;'entity_group': 'ORG', 'score': 0.9647625, 'word': 'Hugging Face', 'start': 33, 'end': 45},
 &amp;lcub;'entity_group': 'LOC', 'score': 0.9986118, 'word': 'Brooklyn', 'start': 49, 'end': 57}]
```
